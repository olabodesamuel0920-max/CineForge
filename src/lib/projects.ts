...
import { Project, CreateProjectInput, PlatformStatus, ProjectDuration, ProjectPlatform } from '@/types/project';
...
export async function getProjects(): Promise<Project[]> {
  if (supabase) {
    try {
      ...
      console.warn('Supabase authentication check failed, using local storage fallback.', e);
    }
  }
...
export async function getProjectById(id: string): Promise<Project | undefined> {
  if (supabase) {
    try {
      ...
      console.warn(`Supabase checks failed for project ${id}, trying local storage fallback.`, e);
    }
  }
