...
import { runNeuralUpscale, runFaceRestoration, MaxQualitySettings } from './aiUpscaler';
...
const maxQualitySettingsSchema = z.object({
  ...
  resolution: z.enum(['720p', '1080p', '4K', '8K', '16K']),
  ...
  faceFidelity: z.number().optional(),
  faceBudgetCap: z.number().optional(),
  faceCacheTtlDays: z.number().optional(),
  identityPreservationMode: z.string().optional()
});
...
renderQueueManager.executeRenderRunner = async (job: RenderJob) => {
  ...
  let payload: RenderRequest | null = null;
  let mqSettings: MaxQualitySettings | null = null;
  let encoderPathUsed: 'QSV' | 'Software' | 'Unknown' = 'Unknown';
  let estOutputSize = 0;
  ...
  try {
    ...
    if (localSoundtrackPath && fs.existsSync(localSoundtrackPath)) {
      ...
      const transientsAnalysis = await extractAudioTransients(localSoundtrackPath);
    ...
    if (mqSettings?.faceRestoration) {
      ...
      try {
        const result = await runFaceRestoration(
          localInputPath,
          faceRestoredPath,
          mqSettings,
        ...
        faceDiagnostics.fallbackReason = (err as Error).message;
        const sharpenedPath = path.join(tmpDir, `input-sharpened-${job.jobId}.mp4`);
        try {
          await runCommandAsync(getFfmpegCommand(), [
            '-y',
