...
exports.soundDesignSettingsSchema = exports.soundEventSchema = exports.AUDIO_ASSETS = exports.soundDesignSchemaVersion = void 0;
exports.parseSoundDirection = parseSoundDirection;
exports.serializeSoundDirection = serializeSoundDirection;
exports.validateAndNormalizeEvents = validateAndNormalizeEvents;
exports.compileSoundDesignPlan = compileSoundDesignPlan;
const zod_1 = require("zod");
exports.soundDesignSchemaVersion = 1;
// Master Asset Manifest - Frontend and Worker use this to resolve asset IDs
exports.AUDIO_ASSETS = [
    // Music beds
...
exports.soundEventSchema = zod_1.z.object({
...
exports.soundDesignSettingsSchema = zod_1.z.object({
 ...
 */
function parseSoundDirection(soundDirection, defaultPresetDirection) {
    ...
    try {
        ...
        if (trimmed.startsWith('{') && trimmed.endsWith('}')) {
            ...
            if (parsed && typeof parsed === 'object' && parsed.schemaVersion === exports.soundDesignSchemaVersion) {
                return {
 ...
 */
function serializeSoundDirection(direction, settings, events) {
    const envelope = {
        schemaVersion: exports.soundDesignSchemaVersion,
        direction,
        settings,
        events
    };
 ...
 */
function validateAndNormalizeEvents(events, totalDuration) {
    const validated = [];
    // Cap at 24 events max to prevent oversized FFmpeg graphs
    const rawEvents = (events || []).slice(0, 24);
    for (const ev of rawEvents) {
        try {
            const parsed = exports.soundEventSchema.parse(ev);
 ...
 */
function compileSoundDesignPlan(timelineBlocks, analysis, preset, settings) {
    if (!settings.enabled) {
        return [];
    }
    const events = [];
    ...
    if (settings.intensity === 'subtle')
        volumeMult = 0.5;
    if (settings.intensity === 'aggressive')
        volumeMult = 1.5;
    ...
    const musicAssetExists = exports.AUDIO_ASSETS.some(a => a.id === musicAssetId);
    ...
    timelineBlocks.forEach((block, idx) => {
        ...
        const isClimax = !isFirst && !isLast && ((block.title || '').toLowerCase().includes('climax') ||
