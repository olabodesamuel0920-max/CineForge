...
exports.runFaceRestoration = runFaceRestoration;
...
function cropRgbaImage(rgbaData, frameWidth, frameHeight, xMin, yMin, cropWidth, cropHeight) {
...
function resizeRgbaImage(srcData, srcW, srcH, destW, destH) {
...
async function callReplicateGFPGAN(dataUri, token) {
    const response = await fetch('https://api.replicate.com/v1/predictions', {
        method: 'POST',
        headers: {
            'Authorization': `Token ${token}`,
...
async function reassembleVideo(framesDir, originalVideoPath, outputVideoPath) {
...
async function runFaceRestoration(localInputPath, localOutputPath, settings, outputGcsUrl, tmpDir, projectDuration, jobId, rawFileHash) {
    const startTime = Date.now();
    ...
    try {
        ...
        if (token === 'mock-token') {
            ...
            try {
                await (0, gcs_1.uploadToGcs)(localOutputPath, gcsCacheUri, 'video/mp4', {
        ...
        return {
            ...
            facesDetected: 0,
            framesProcessed: 0,
            fallbackReason: err.message || 'Processing error',
            fidelitySetting: fidelity,
            privacyCacheTtlDays: ttlDays
        };
    }
